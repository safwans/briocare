...
/** Status strip under the room: what the AI teens are doing, and anything that's in their way. */
export function SimStatusBar() {
  const { active, mode, asr, error, lines, riskRehearsal, setRiskRehearsal, stopAll } = useSim();
  if (active.length === 0 && !error) return null;

  return (
    <div className="mt-3 rounded-xl p-4" style={{ background: "#12303a", border: "1px solid #1e4a56" }}>
      <div className="flex items-center gap-3 flex-wrap">
        <span className="text-[11px] font-bold uppercase tracking-wider" style={{ color: "#fcd34d" }}>
          🤖 AI patients
        </span>
        <span className="text-[11px]" style={{ color: "#6a8b93" }}>
          dev only · they really join the room, so use headphones · stop them before editing LiveRoom.tsx
        </span>
        <div className="ml-auto flex items-center gap-3">
          <label className="text-[12px] flex items-center gap-1.5" style={{ color: "#9db8bd" }}>
            <input type="checkbox" checked={riskRehearsal} onChange={(e) => setRiskRehearsal(e.target.checked)} />
            Rehearse a risk flag
          </label>
          {active.length > 0 && (
            <button
              onClick={stopAll}
      ...
      {error && (
        <p className="text-[12px] mt-2" style={{ color: "#f0a3b1" }}>
          {error}
        </p>
      )}
      ...
      {active.length > 0 && (
        <>
          <div className="flex items-center gap-2 mt-3 text-[12px]" style={{ color: "#7fd0dc" }}>
            <span
              className="w-2 h-2 rounded-full"
              style={{ background: mode === "speaking" ? "#ef7d5a" : mode === "thinking" ? "#e8c05a" : "#4bbf93" }}
            />
            {mode === "thinking" ? "Thinking…" : mode === "speaking" ? "Speaking…" : "Listening — say something out loud"}
            <span style={{ color: "#5f8089" }}>· {active.length} simulated</span>
          </div>

          {asr !== "on" && (
            <p className="text-[12px] mt-2" style={{ color: "#e8c05a" }}>
              {asr === "unknown"
                ? "Waiting for live transcription to start — until it does, the teens can't hear you and will only chat among themselves."
                : "Live transcription isn't running on this room, so the teens can't hear you. They'll still talk on their own, and the session still produces notes — but ask-a-question-get-an-answer needs Daily transcription enabled on your domain."}
            </p>
          )}

          {/* Deliberately no line-by-line log here. This panel used to print every turn, which read
              as a second transcript sitting under the real one in the sidebar — two lists of the
              same conversation, worded differently (this one shows the script the bot was handed,
              the sidebar shows what ASR actually heard) and only one of them surviving a page
              navigation. The sidebar transcript is the single place a conversation is shown. */}
          {lines.length > 0 && (
            <p className="text-[12px] mt-2" style={{ color: "#5f8089" }}>
              {lines.length} turn{lines.length === 1 ? "" : "s"} so far — see the Transcript panel above.
            </p>
          )}
        </>
      )}
